...
| `operational-configuration/AlertConfigurationResolver.js` | Añade `resolveResourceAlertConfiguration(resource)` y `extractResourceAlertMetadata(resource)` → **dueño oficial de lectura**: extrae `alertConfiguration ?? alert_config`, normaliza, devuelve `{source:'metadata'|'default', resourceId, config}`. `resolveOperationalConfiguration` intacto. |

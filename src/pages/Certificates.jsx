...
import { 
  ...
  Loader2, ShieldCheck, Calendar, User, Search, 
  Beaker, Package, Factory, Briefcase, ChevronRight
} from 'lucide-react';
...
export default function Certificates() {
  ...
  const loadRecords = async () => {
    try {
      setLoading(true);
      const data = await documentsService.getRecords('trazabilidad', null); // Traer todos los tipos
      setRecords(data);
    } catch (error) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      setUploading(true);
      const newRecord = await documentsService.uploadRecord('trazabilidad', selectedCategory, file, user.id);
      setRecords(prev => [newRecord, ...prev]);
      setSelectedCategory(null);
      alert('Certificado guardado correctamente en ' + CATEGORIES.find(c => c.id === selectedCategory)?.name);
    } catch (error) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-white p-8 rounded-3xl border border-gray-200 shadow-sm">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Gestión documental organizada por categorías técnicas.</p>
        </div>

        {isAdmin && (
          <div className="flex flex-col gap-2 w-full sm:w-auto">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">Subir nuevo certificado en:</label>
      ...
      <div className="relative max-w-2xl mx-auto">
        ...
        <input 
          type="text" 
          placeholder="Buscar certificados en todas las categorías..." 
          value={searchTerm}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-gray-500 animate-pulse font-medium">Organizando repositorio por categorías...</p>
        </div>
      ) : (
                ...
                <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Repositorio de documentos técnicos</p>
                    </div>
                  </div>

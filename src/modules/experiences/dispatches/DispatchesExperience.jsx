 ...
 *
 * Sprint 79 — First Operational Experience.
 *
 * Reusable dispatch management experience that can be plugged into any module
 * via the 'operational-experiences' capability.
 *
 * Wraps existing dispatches logic (despachosService, dispatchesPdf, etc.)
 * without duplicating any business logic.
 *
 * Props:
 *   moduleSlug — the slug of the host module (for context/display)
 *   moduleName — the display name of the host module
 */
...
import {
  ...
  Calendar, Edit2, Trash2, AlertTriangle
} from 'lucide-react';
...
export default function DispatchesExperience({ moduleSlug, moduleName }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      if (!isSupabaseConfigured()) {
        ...
        setBanner({
          type: 'error',
          message: 'Supabase no configurado: cree un archivo .env con VITE_SUPABASE_URL y VITE_SUPABASE_ANON_KEY.',
        });
        return;
      }
  ...
  const [formData, setFormData] = useState({
    fecha: '', hora: '', cliente: '', producto: '', presentacion: '',
    lote: '', cantidad: '', peso: '', destino: '', temperatura: '',
    calidadEmpaque: 'Excelente', placa: dispatchDefaults.placa,
    conductor: dispatchDefaults.conductor, firmaConductor: '', observaciones: ''
  });
  ...
  useEffect(() => {
    if (isFormOpen) {
      if (editingRecord) {
        setFormData({
          fecha: editingRecord.fecha, hora: editingRecord.hora,
          cliente: editingRecord.cliente, producto: editingRecord.producto,
          presentacion: editingRecord.presentacion || '', lote: editingRecord.lote,
          cantidad: editingRecord.cantidad, peso: editingRecord.peso,
          destino: editingRecord.destino, temperatura: editingRecord.temperatura || '',
          calidadEmpaque: editingRecord.calidadEmpaque || 'Excelente',
          placa: editingRecord.placa, conductor: editingRecord.conductor,
        ...
        const now = new Date();
        setFormData({
          fecha: format(now, 'yyyy-MM-dd'), hora: format(now, 'HH:mm'),
          cliente: '', producto: '', presentacion: '', lote: '',
          cantidad: '', peso: '', destino: '', temperatura: '',
          calidadEmpaque: 'Excelente', placa: dispatchDefaults.placa,
          conductor: dispatchDefaults.conductor, firmaConductor: '', observaciones: ''
        });
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setBanner({ type: 'error', message: err?.message || 'No se pudo guardar el despacho.' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleExcelImported = async (rows) => {
    ...
    try {
      const importedUi = (rows || []).map((r) => ({
        fechaDespacho: r.fechaDespacho || '', hora: r.hora || '',
        cliente: r.cliente || '', producto: r.producto || '',
        lote: r.lote || '', cantidadBolsas: r.cantidadBolsas ?? '',
        peso: r.peso ?? '', destino: r.destino || '',
      ...
      setBanner({ type: 'success', message: `Importación exitosa: ${inserted.length} registros guardados.` });
    } catch (err) {
      setBanner({ type: 'error', message: err?.message || 'Ocurrió un error al importar el Excel.' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      setBanner({ type: 'error', message: err?.message || 'No se pudo generar el PDF.' });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Registro diario de vehículos, productos y conductores.
          </p>
        </div>
            ...
            <>
              <RoleGate allowedRoles={['administrador', 'calidad']}>
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-xl font-medium border border-gray-200 transition-colors text-sm disabled:opacity-50"
                >
              ...
              <RoleGate allowedRoles={['administrador', 'operativo', 'calidad']}>
                <button
                  ...
                  disabled={loadingRecords || saving}
                  className="flex items-center justify-center gap-2 px-5 py-2 bg-primary hover:bg-primary-light text-white rounded-xl font-bold transition-all shadow-md shadow-primary/20 text-sm disabled:opacity-50"
                >
      ...
      {banner && (
        <div className={`rounded-2xl border px-5 py-4 text-sm flex items-start gap-3 ${
          ...
          <button onClick={() => setBanner(null)} className="p-1.5 rounded-lg hover:bg-black/5 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      ...
      {isFormOpen ? (
          ...
          <form onSubmit={handleSubmit} className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-6">
              ...
              <div className="col-span-2 space-y-1.5">
                ...
                <input list={`exp-products-${moduleSlug}`} value={formData.producto} onChange={handleProductChange} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm font-medium text-gray-900" placeholder="Escriba producto..." required />
                <datalist id={`exp-products-${moduleSlug}`}>
              ...
              <div className="col-span-2 space-y-1.5">
                ...
                <input list={`exp-drivers-${moduleSlug}`} value={formData.conductor} onChange={handleDriverChange} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm font-medium text-gray-900" placeholder="Seleccione conductor..." required />
                <datalist id={`exp-drivers-${moduleSlug}`}>
            ...
            <div className="mt-10 flex items-center justify-end gap-3 border-t pt-6">
              ...
              <button type="submit" disabled={saving} className="flex items-center gap-2 px-8 py-3 bg-primary hover:bg-primary-light text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-primary/20 disabled:opacity-50">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50">
            ...
            <button className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse min-w-[1000px]">
              ...
              <tbody className="divide-y divide-gray-100">
                {loadingRecords ? (
                  <tr><td colSpan={7} className="p-10 text-center text-sm text-gray-500">Cargando despachos desde Supabase…</td></tr>
                  ...
                  filteredRecords.map((record) => (
                    <tr key={record.id} className="hover:bg-primary/[0.02] transition-colors group">
                      ...
                      <td className="p-4 text-sm font-medium text-gray-900">{record.cantidad}</td>
                      <td className="p-4">
                      ...
                      <td className="p-4 pr-6 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <RoleGate allowedRoles={['administrador']}>
                            <button onClick={() => handleEdit(record)} className="p-1.5 text-gray-400 hover:text-primary bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow transition-all" title="Editar registro">
                              <Edit2 className="w-4 h-4" />
                            </button>
                          </RoleGate>
                          <RoleGate allowedRoles={['administrador']}>
                            <button onClick={() => handleDelete(record.id)} className="p-1.5 text-gray-400 hover:text-red-600 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow transition-all" title="Eliminar registro">
